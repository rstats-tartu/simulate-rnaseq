...
rule simulate_experiment:
    ...
    params:
        reads_per_transcript = lambda wildcards, input: reads_per_transcript(input.fasta, 20),
        fold_changes = fold_changes([0.5,1,2], prob=[0.05,0.9,0.05], n_groups=N_GROUPS, n_transcripts=N_TX, seed = config["fold_changes"]["seed"]), 
